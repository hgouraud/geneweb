...
let capitale w = String.capitalize w
...
let exec_wait conf prog args =
  let wnd =
    GWindow.window ~title:(capitale (transl "Processing")) ~position:`CENTER
  ...
  let scrolled_window =
    GBin.scrolled_window ~border_width:10 ~hpolicy:`AUTOMATIC
      ~vpolicy:`AUTOMATIC ~packing:vbox#add ()
  in
  let vvbox =
...
let error_popup msg =
  let wnd =
    GWindow.window ~title:(capitale (transl "error")) ~position:`CENTER
      ~resizable:true ()
  in
  ignore (wnd#connect#destroy ~callback:(fun _ -> ()));
  let vbox = GPack.vbox ~spacing:5 ~packing:wnd#add () in
  ...
  let bbox =
    GPack.button_box `HORIZONTAL ~border_width:5 ~layout:`SPREAD
      ~packing:vbox#pack ()
  in
...
let display_log conf =
  let wnd =
    GWindow.window ~title:(capitale (transl "log")) ~position:`CENTER
      ~resizable:true ~width:600 ~height:300 ()
  in
  ignore (wnd#connect#destroy ~callback:(fun _ -> ()));
  let vbox = GPack.vbox ~spacing:5 ~packing:wnd#add () in
  ...
  let bbox =
    GPack.button_box `HORIZONTAL ~border_width:5 ~layout:`EDGE
      ~packing:vbox#pack ()
  in
  let btn_ok = GButton.button ~label:(transl "OK") ~packing:bbox#pack () in
  ignore (btn_ok#connect#clicked (fun () -> wnd#destroy ())); wnd#show ()
...
let delete_base conf bname =
  let wnd =
    GWindow.window ~title:(capitale (transl "Confirm")) ~position:`CENTER
      ~resizable:true ~width:300 ~height:50 ()
  in
  let vbox = GPack.vbox ~spacing:5 ~packing:wnd#add () in
  let bbox =
    GPack.button_box `HORIZONTAL ~border_width:5 ~layout:`SPREAD
  ...
  if bname = "" then ()
    ...
    begin
      if src_file = "" then gwc2 conf bname src_file
        ...
        begin let fname = String.lowercase src_file in
...
let main_window =
  ignore (GMain.init ());
  let wnd =
    GWindow.window ~title:("GeneWeb - " ^ Version.txt) ~position:`CENTER
      ~resizable:true ~width:640 ~height:480 ()
  in
...
let rec show_main conf =
  ...
  let inser_toolbar text tooltip icon_file callback =
    toolbar#insert_button ~text:text ~tooltip:tooltip
      ~tooltip_private:"Private" ~icon:(icon icon_file) ~callback:callback ()
  in
  ignore
  ...
  *)
  ...
  if databases = [] then
    ignore
      (GMisc.label ~text:(capitale (transl "no databases."))
         ~packing:vbox#pack ())
  else
    let _label =
      GMisc.label
    ...
    vvbox#focus#set_vadjustment (Some scrolled_window#vadjustment);
    let table =
      GPack.table ~rows:3 ~columns:3 ~row_spacings:5 ~col_spacings:5
        ~packing:vvbox#pack ()
    in
    let rec loop i =
      function
      ...
      | bname :: l ->
          ...
          in
          let bbut =
            GButton.button ~label:(capitale (transl "browse"))
          ...
          ignore
            (bbut#connect#clicked
               (fun () ->
                  ...
                  ignore (browse conf url)));
          let bbut =
            GButton.button ~label:(capitale (transl "tools"))
...
and new_database conf =
  ...
  let table =
    GPack.table ~rows:2 ~columns:2 ~row_spacings:5 ~col_spacings:5
      ~packing:vbox#pack ()
  in
  let _label =
    GMisc.label ~text:(capitale (transl "name of the database:"))
  ...
  in
  let _label =
    GMisc.label ~text:(capitale (transl "select a file"))
...
and setup_gui conf =
  ...
  let hbox = GPack.hbox ~spacing:5 ~packing:vbox#pack () in
  let _label =
    GMisc.label ~text:"Port :"
      ~packing:(hbox#pack ~expand:false ~fill:false ~padding:5) ()
  in
  let entry =
    GEdit.entry ~text:(string_of_int conf.port)
      ~packing:(hbox#pack ~expand:false ~fill:false ~padding:5) ()
  in
  ignore
...
and config_gwf conf bname =
  let vbox = GPack.vbox ~spacing:5 ~packing:main_window#add () in
  let _label =
    GMisc.label ~text:(transl "Configuration gwf file of " ^ bname)
      ~packing:vbox#pack ()
  in
  let scrolled_window =
    GBin.scrolled_window ~border_width:10 ~hpolicy:`AUTOMATIC
...
and tools conf bname =
  ...
  in
  let _label =
    GMisc.label ~text:(transl "extract gw file")
      ~packing:(table#attach ~left:0 ~top:0) ()
  in
  let bbut =
    GButton.button ~label:(transl "Extract GW")
      ~packing:(table#attach ~left:1 ~top:0) ()
  in
  ignore (bbut#connect#clicked (fun () -> tmp_wnd conf bname gwu));
  let _label =
    GMisc.label ~text:(transl "extract ged file")
      ~packing:(table#attach ~left:0 ~top:1) ()
  in
  let bbut =
    GButton.button ~label:(transl "Extract GED")
      ~packing:(table#attach ~left:1 ~top:1) ()
  in
  ignore (bbut#connect#clicked (fun () -> tmp_wnd conf bname gwb2ged));
  let _label =
    GMisc.label ~text:(transl "setup base options")
      ~packing:(table#attach ~left:0 ~top:2) ()
  in
  let bbut =
    GButton.button ~label:(transl "setup gwf file")
      ~packing:(table#attach ~left:1 ~top:2) ()
  in
  ignore
    (bbut#connect#clicked (fun () -> vbox#destroy (); config_gwf conf bname));
  let _label =
    GMisc.label ~text:(transl "Clean database")
      ~packing:(table#attach ~left:0 ~top:3) ()
  in
  let bbut =
    GButton.button ~label:(transl "Clean")
  ...
  in
  let bbut =
    GButton.button ~label:(transl "Rename")
  ...
  in
  let bbut =
    GButton.button ~label:(transl "Delete")
...
let launch_config () =
  if Sys.file_exists config_gui_file then
    ...
    let _label =
      GMisc.label ~text:(transl "select bases directory")
    ...
    let _label =
      GMisc.label ~text:(transl "enter port")
        ~packing:(page_2#pack ~expand:false ~fill:false ~padding:5) ()
    in
    let entry =
      GEdit.entry ~text:(string_of_int !port)
        ~packing:(page_2#pack ~expand:false ~fill:false ~padding:5) ()
    in
    ignore
    ...
    let _label =
      GMisc.label ~text:(transl "select browser")
    ...
    ignore
      (assistant#append_page ~title:(transl "Introduction") ~page_type:`INTRO
         ~complete:true page_0#as_widget);
    ignore
      (assistant#append_page ~title:(transl "Setup bases directory")
         ~page_type:`CONTENT page_1#as_widget);
    ignore
      (assistant#append_page ~title:(transl "Setup port") ~page_type:`CONTENT
         ~complete:true page_2#as_widget);
    ignore
      (assistant#append_page ~title:(transl "Setup browser")
         ~page_type:`CONTENT page_3#as_widget);
    ignore
      (assistant#append_page ~title:(transl "Finnish") ~page_type:`CONFIRM
         ~complete:true page_4#as_widget);
    let save_config () =
      let gui_arg =
