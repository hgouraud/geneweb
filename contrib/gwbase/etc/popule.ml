...
let nameize _ =
  ...
  let len =
    ...
    | _ -> Buff.mstore len c5
  in
  let len = Buff.mstore len (char6 ()) in String.uppercase (Buff.get len)
...
let add_indi (base, cnt, bname) (fn, sn1, sn2, oc) sex =
  ...
  Gwlib.add_indi !base (fn, sn, oc) sex
    ...
    if n > ngen then ()
      ...
      for i = 0 to size - 1 do
        if List.mem i celib then ()
          ...
          in
          let _ = (Gwlib.add_fam !base ifath imoth list : ifam) in ()
      done;
...
let speclist =
  ["-s", Arg.Int (fun i -> size := i),
   ...
   "age at mariage (" ^ string_of_int !gyear ^ ")"]
