      ...
      begin
        ...
        then
          begin
            ...
            if !compile then patch_person base p.key_index p; incr changes
          end;
...
let dead_all bname lim_year titled =
  ...
  if !changes > 0 then commit_patches base

let dead_some bname lim_year titled key =
  let base = Gwdb.open_base bname in
  ...
  | _ ->
      match Gutil.person_of_string_dot_key base key with
        Some ip ->
          let p = poi base ip in
          if get_access p <> Private && !compile then
            begin let p =
              {(gen_person_of_person p) with death = OfCourseDead}
            in
...
let speclist =
  ["-y", Arg.Int (fun i -> lim_year := i),
   ...
   "-co", Arg.Clear compile, "compile only";
   "-ind", Arg.String (fun x -> ind := x), "individual key"]
let anonfun i = bname := i
let usage = "Usage: public1 [-co] [-ct] [-y #] [-ind key] base"
...
let main () =
  ...
  match
    Lock.control (Mutil.lock_file !bname) false
      (fun () ->
         if !ind = "" then dead_all !bname !lim_year !titled
         else dead_some !bname !lim_year !titled !ind;
      ...
      match
        Lock.control (Mutil.lock_file !bname) true
          (fun () ->
             ...
             else if !ind = "" then dead_all !bname !lim_year !titled
             else dead_some !bname !lim_year !titled !ind)
      with
        Some x -> x
      | None ->
