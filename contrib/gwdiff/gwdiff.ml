...
let print_f_messages base1 base2 ifam1 ifam2 res =
  ...
  Printf.printf "%s x %s%s/ %s x %s%s"
    (person_link !in_file1 base1 c1.father "base1")
    (person_link !in_file1 base1 c1.mother "base1") !cr
    (person_link !in_file2 base2 c2.father "base2")
    (person_link !in_file2 base2 c2.mother "base2") !cr;
...
let dmy_to_sdn_range_l dmy =
  ...
  | OrYear y ->
      let dmy2 = {year = y + 1; month = 0; day = 0; prec = Sure; delta = 0} in
  ...
  | YearInt y ->
      let dmy2 = {year = y + 1; month = 0; day = 0; prec = Sure; delta = 0} in
      let (sdn11, sdn12) = sdn_of_dmy dmy in
      let (sdn21, sdn22) = sdn_of_dmy dmy2 in [Some sdn11, Some sdn22]
...
let compatible_unions base1 base2 iper1 iper2 ifam1 ifam2 =
  let get_spouse base iper ifam =
    ...
    if iper = c.father then poi base c.mother else poi base c.father
  in
...
let rec ddiff base1 base2 iper1 iper2 d_tab =
  ...
  if List.mem iper2 d_check then ()
    ...
    let spouse c iper = if iper = c.father then c.mother else c.father in
...
let main () =
  ...
  (* Reference base *)
  let base1 = Iobase.input !in_file1 in
  ...
  (* Destination base *)
  let base2 =
    if !in_file1 != !in_file2 then
      let base2 = Iobase.input !in_file2 in
  ...
  in
  let iper1 = person_ht_find_unique base1 !p1_fn !p1_sn !p1_occ in
  let iper2 = person_ht_find_unique base2 !p2_fn !p2_sn !p2_occ in
  if !html then Printf.printf "<BODY>\n";
  gwdiff base1 base2 iper1 iper2 !d_mode !ad_mode;
  if !html then Printf.printf "</BODY>\n"
