...
let reorder (conf : Config.config) url_env =
  ...
  let unordered_params =
    List.fold_left
      (fun acc (k, v) ->
        if List.mem k processed_keys || not (should_include_param k v) then
          acc
        else
...
let url_set_aux conf url evar_l str_l =
  ...
  let str_l = 
    List.mapi (fun i _ -> 
      if i < List.length str_l then List.nth str_l i else ""
    ) evar_l
  in
  ...
  let url_env =
    List.fold_left2
      (fun acc evar str ->
        if str <> "" then (evar, str) :: acc else acc
      )
      []
      evar_l
      str_l
