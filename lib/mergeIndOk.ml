...
let effective_mod_merge o_conf base o_p1 o_p2 sp =
  ...
  | Some i2 ->
      ...
      delete_key base
        (sou base (get_first_name p2))
        (sou base (get_surname p2))
        (get_occ p2);
      let warning _ = () in
      ...
      patch_cache_info conf Util.cache_nb_base_persons (fun v ->
      ...
      print_mod_merge_ok conf base wl p pgl1 ofn1 osn1 oocc1 pgl2 ofn2 osn2
        oocc2
