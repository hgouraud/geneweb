...
and eval_dag_cell_var conf base env (colspan, align, td) = function
  ...
  | [ "father"; "Util.access" ] ->
      parents_access_aux conf base td Driver.get_father
  | [ "has_next_sibling" ] -> (
  ...
  | [ "item" ] -> (
      ...
      | _ -> VVstring "")
  | [ "mother"; "Util.access" ] ->
      parents_access_aux conf base td Driver.get_mother
  | [ "next_sibling"; "Util.access" ] -> sibling_access_aux conf base td true
  | [ "prev_sibling"; "Util.access" ] -> sibling_access_aux conf base td false
  | [ "text" ] -> (
      match td with
