...
let make_conf cgi from_addr request script_name env =
  ...
  let conf =
    {from = from_addr;
     ...
     user = ar.ar_user; 
     username = ar.ar_user_name;
     (* FIXME userkey, userpwd in cgi mode *)
     userkey = if cgi then "" else ar.ar_user_key;
     userpwd = if cgi then "" else ar.ar_user_pwd;
...
let conf_and_connection cgi from request script_name contents env =
  let (conf, sleep, passwd_err) = make_conf cgi from request script_name env in
      ...
      if not accept then only_log from
        ...
        try
          ...
          if image_request script_name env then ()
            ...
            conf_and_connection cgi from request script_name contents env
        with
...
let geneweb_server () =
  ...
  Wserver.f !selected_addr !selected_port !conn_timeout
    (if Sys.unix then !max_clients else None) (connection false)
