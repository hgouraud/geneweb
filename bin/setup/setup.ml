(* DO NOT EDIT *)
(* This file was generated from bin/setup/setup.camlp5.ml *)
open Geneweb
    ...
    function
      (k, s) :: env ->
        ...
        if k = "" || s = "" then loop comm bname env
          ...
          let out = stringify s in comm ^ " -o " ^ out ^ " > " ^ out
        else if k = "o" && s <> "" then
          if s = "choice" then loop comm bname env
          else
            let out = stringify s in
            let out =
              if out = "/notes_d/connex.txt" then bname ^ ".gwb" ^ out
              else out
            in
      ...
      | '%' ->
          ...
          | 'A'..'Z' | '0'..'9' as c ->
              ...
              | 'I' ->
                  ...
                  print_if_else conf print (p_getenv conf.env k1 = Some k2)
                    strm
              | 'O' ->
                  let fname =
                    Filename.remove_extension
                      (Filename.basename
                         (strip_spaces (s_getenv conf.env "o")))
                  in
                  let fname = slashify_linux_dos fname in print fname
              | 'P' -> print (string_of_int !gwd_port)
              | 'Q' -> print (parameters_1 conf.env)
              | 'R' -> print (parameters_2 conf.env)
              | 'S' -> print (parameters conf.env)
              | _ ->
        ...
        List.iter
          (fun (k, v) ->
             if k <> "sel" && k <> "body_prop" then
               begin print k; print "="; print v; print ";" end)
          conf.env;
        print "sel=";
        print (code_varenv d);
        print "\">";
...
let gwc_or_ged2gwb out_name_of_in_name conf =
  ...
  if in_file = "" || out_file = "" then print_file conf "err_miss.htm"
    ...
    not (Sys.file_exists in_file) && not (String.contains fname '*')
  then
    print_file conf "err_unkn.htm"
  else if not (good_name out_file) then print_file conf "err_name.htm"
  else print_file conf "bso.htm"
...
let gwfixbase ok_file conf =
  ...
  Printf.eprintf "\n";
  flush stderr;
  if rc > 1 then print_file conf "fix_err.htm"
  else
    let conf =
      conf_with_env conf "o" (Filename.basename (s_getenv conf.env "o"))
    in
...
let delete_1 conf =
  List.iter (fun (k, v) -> if v = "del" then Util.rm_rf (k ^ ".gwb"))
    conf.env;
...
let setup_comm_ok conf =
  ...
  | x ->
      if Mutil.start_with "doc/" 0 x || Mutil.start_with "images/" 0 x ||
