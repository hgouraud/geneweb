...
let print_search_form conf from_note =
  ...
  begin match from_note with
    Some n ->
      Wserver.wprint "<input type=\"hidden\" name=\"z\" value=\"%s\"%s>\n" n
        conf.xhs
  | None -> ()
  end;
  Wserver.wprint "<br%s>\n" conf.xhs;
  Wserver.wprint "<label>\n";
  Wserver.wprint "<input type=\"checkbox\" name=\"c\" value=\"on\"%s%s>\n"
...
let print_whole_notes conf base fnotes title s ho =
  ...
  let what_links_page () =
    if fnotes <> "" then
      begin
        Wserver.wprint "<a href=\"%sm=NOTES;f=%s;ref=on\">" (commd conf)
          fnotes;
  ...
  List.iter
    (fun pg ->
       ...
       | NotesLinks.PgNotes ->
           ...
           Wserver.wprint "<img src='%s/picto_molette.png'>"
             (Util.image_prefix conf);
           Wserver.wprint "</a> ";
           Wserver.wprint "<a href=\"%sm=NOTES\">" (commd conf);
           Wserver.wprint "%s" (transl_nth conf "note/notes" 1);
           Wserver.wprint "</a>\n"
       | NotesLinks.PgMisc fnotes ->
           Wserver.wprint "<a href='%sm=MOD_NOTES;f=%s'>" (commd conf) fnotes;
           Wserver.wprint "<img src='%s/picto_molette.png'>"
             (Util.image_prefix conf);
           Wserver.wprint "</a> ";
           Wserver.wprint "<tt>";
           Wserver.wprint "[";
           Wserver.wprint "<a href=\"%sm=NOTES;f=%s\">" (commd conf) fnotes;
           Wserver.wprint "%s" fnotes;
           Wserver.wprint "</a>";
           Wserver.wprint "]";
           Wserver.wprint "</tt>\n"
       | NotesLinks.PgWizard wizname ->
           Wserver.wprint "<a href='%sm=MOD_WIZNOTES;f=%s'>" (commd conf)
             (code_varenv wizname);
           Wserver.wprint "<img src='%s/picto_molette.png'>"
             (Util.image_prefix conf);
           Wserver.wprint "</a> ";
           ...
           Wserver.wprint "</i>";
           Wserver.wprint " ";
           Wserver.wprint "<a href=\"%sm=WIZNOTES;v=%s\">" (commd conf)
             (code_varenv wizname);
...
let print_misc_notes conf base =
  ...
  if db <> [] then
    begin
      ...
      if d <> "" then
        begin
          Wserver.wprint "<li class=\"parent\">\n";
          begin
            Wserver.wprint "<a href=\"%sm=MISC_NOTES%s\">" (commd conf)
              (try
        ...
        (fun (r, f) ->
           match f with
             Some f ->
               ...
               in
               Wserver.wprint "<li class=\"file\">\n";
               Wserver.wprint "<tt>[";
               Wserver.wprint "<a href=\"%sm=NOTES;f=%s\"%s>" (commd conf) f
                 c;
               Wserver.wprint "%s" r;
               Wserver.wprint "</a>";
               Wserver.wprint "]</tt>%s\n"
               ...
               Wserver.wprint "</li>\n"
           | None ->
               Wserver.wprint "<li class=\"folder\">\n";
               Wserver.wprint "<tt>";
               Wserver.wprint "<a href=\"%sm=MISC_NOTES;d=%s\">" (commd conf)
                 (if d = "" then r
...
let cache_fname_person_linked_pages conf =
  let bname =
...
let read_cache_person_linked_pages conf =
...
let write_cache_person_linked_pages conf =
  ...
  match try Some (Secure.open_out_bin fname) with Sys_error _ -> None with
    Some oc -> output_value oc ht_cache_person_linked_pages; close_out oc
  | None -> ()

let patch_cache_person_linked_pages conf k v =
  if v then
    begin
...
let has_linked_pages conf base ip db =
