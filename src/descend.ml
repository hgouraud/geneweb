...
let display_descendants_level conf base max_level ancestor =
  ...
  print_alphab_list conf
    ...
    (fun (p, c) ->
       ...
       if not (is_hidden p) && c > 1 then Wserver.wprint " <em>(%d)</em>" c;
       Wserver.wprint "\n")
    list;
  trailer conf
...
let print_child conf base p1 p2 e =
  ...
  then
    Wserver.wprint "%s" (referenced_person_text_without_surname conf base e)
...
let display_descendants_with_numbers conf base max_level ancestor =
  ...
  if !total > 1 then
    begin
      html_p conf;
      Wserver.wprint "%s: %d %s" (capitale (transl conf "total")) !total
        (Util.translate_eval ("@(c)" ^ transl_nth conf "person/persons" 1));
      if max_level > 1 then
  ...
  for i = 0 to nb_of_persons base - 1 do
    if paths.(i) <> [] then
      ...
      if p_first_name base p <> "?" && p_surname base p <> "?" &&
         ...
         (not (is_hide_names conf p) || fast_auth_age conf p)
      then
...
let print_desc_table_header conf base =
  let nb_col = ref 2 in
  Wserver.wprint "<tr class=\"descends_table_header\">\n";
  Wserver.wprint "<th>\n";
...
let print_person_table conf base p lab =
  ...
  if p_getenv conf.env "child" = Some "on" then
    ...
    then
      begin
        Wserver.wprint "<td align=\"center\" %s>\n"
          (if nb_families > 1 then "style=\"border-bottom:none\"" else "");
        begin let u = p in
          if nb_families > 0 then
            let fam = foi base (get_family u).(0) in
  ...
  if nb_families > 1 then
    ...
    then
      ...
      for i = 1 to nb_families - 1 do
        ...
        if p_getenv conf.env "marr" = Some "on" then
          begin
            Wserver.wprint "<td style=\"border-top:none; %s\">\n"
...
let display_descendant_with_table conf base max_lev p =
    ...
    | (p, lab) :: q ->
        if first && lev > 0 && p_getenv conf.env "gen" = Some "on" then
          begin
            Wserver.wprint "<tr>\n";
            begin
              Wserver.wprint "<th align=\"left\" colspan=\"%d\">\n" nb_col;
              Wserver.wprint "%s %d"
  ...
  Wserver.wprint "</p>\n";
  Wserver.wprint "<table class=descends_table>\n";
  ...
  Wserver.wprint "<p>\n";
  Wserver.wprint "%s: %d %s" (capitale (transl conf "total")) !nb_pers
        ...
        let rec loop tdl i =
          if i = Array.length (get_family p) then tdl
            ...
            let td =
              ...
              let s =
                if bd > 0 || td_prop <> "" then
                  Printf.sprintf "<table border=\"%d\"><tr>\
