          ...
          begin
            ...
            begin
              ...
              begin
                Wserver.wprint "<span>";
                begin
                  Wserver.wprint "<a href=\"%s;m=R;%s\">" (commd conf)
                    (acces conf base p1);
                  Wserver.wprint "%s"
                    (capitale
...
let print_link_name conf base n p1 p2 sol =
  ...
  if n > 1 then Wserver.wprint " %s" (transl conf "also");
  Wserver.wprint "\n";
  let (s, sp1, sp2) =
    let ini_p1 = p1
    and ini_p2 = p2 in
       ...
       if not long then
         begin let propose_dag = n > 1 && n <= 10 in
           ...
           let dp2 =
             ...
             | _ -> p2
           in
           Wserver.wprint "<img src=\"%s/%s\" alt=\"\"%s>\n"
...
let print_solution_not_ancestor conf base long p1 p2 sol =
  ...
  let image_opt =
    ...
    | _ -> ""
  in
  Wserver.wprint "<ul class=li_relationship>\n";
  Wserver.wprint "<li>\n";
    ...
    M.iter
      (fun ip (pp1, pp2, nn, nt, maxlev) ->
         ...
         if nt > 1 && nn > 1 && nn < max_br then
           ...
           Wserver.wprint ";l1=";
           begin let _ =
             List.fold_right (fun x sep -> Wserver.wprint "%s%d" sep x; ",")
               l1 ""
           in
             Wserver.wprint ";l2=";
             let _ =
