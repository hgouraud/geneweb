...
let is_access_consent conf base p =
...
let is_hide_names_full conf base p =
  let is_access_c =
    if conf.friend && conf.half_rgpd then true
    else is_access_consent conf base p
  in
  if conf.wizard || get_access p = Public then false
  else if conf.friend && get_access p = Private && not conf.half_rgpd then
    true
  else if
    conf.friend && get_access p = Friend || conf.friend && is_access_c
  then
    false
  else true
...
let is_hide_names conf p =
  if conf.wizard || get_access p = Public ||
     conf.friend && get_access p = Friend || conf.friend && conf.half_rgpd
  then
    false
  else true
...
let add_doc_path = Secure.add_doc_path
...
let search_in_doc_path = search_in_path Secure.doc_path
...
let begin_centered conf =
  Wserver.wprint
    ...
    conf.border
let end_centered _ = Wserver.wprint "</td></tr></table>\n"

let html_br conf = Wserver.wprint "<br%s>\n" conf.xhs

let html_p conf = Wserver.wprint "<p>"; Wserver.wprint "\n"

let html_li conf = Wserver.wprint "<li>"; Wserver.wprint "\n"
...
let html conf =
  ...
  Wserver.wprint "Date: %s" (string_of_ctime conf);
  nl ();
...
(* ************************************************************************** *)
let redirect_HTML conf status_code url =
...
let is_old_person conf p =
  (match p.death with
     DontKnowIfDead -> p.access <> Private && conf.public_if_no_date
   | OfCourseDead -> true
   | _ -> false) ||
  ...
  (match Adef.od_of_codate p.baptism with
     Some (Dgreg (d, _)) ->
       ...
       strictly_after_private_years conf conf.private_years a
   | _ -> false) ||
  (match CheckItem.date_of_death p.death with
     Some (Dgreg (d, _)) ->
       let a = CheckItem.time_elapsed d conf.today in
...
let fast_auth_age conf p =
  if conf.friend && get_access p = Friend || conf.wizard ||
     get_access p = Public
  then
    true
...
let is_restricted (conf : config) base ip =
  let fct p =
    ...
    not (is_quest_string (get_first_name p)) && not (fast_auth_age conf p)
  in
...
let authorized_age conf base p =
  if not conf.wizard && get_access p = Private then false
    ...
    conf.wizard || conf.friend && is_access_consent conf base p ||
    get_access p = Public
  then
    true
  else if
    conf.public_if_titles && get_access p = IfTitles &&
    (nobtit conf base p <> [] || parent_has_title conf base p)
  then
    true
  else
...
let base_len n =
  ...
  match try Some (Gwdb.open_base n) with Sys_error _ -> None with
    Some base ->
      let len = nb_of_persons base in close_base base; string_of_int len
  | _ -> "?"
...
let macro_etc env imcom c =
  try List.assoc c env () with
    Not_found ->
      match c with
        '%' -> "%"
      | 'k' -> imcom
      | 'o' -> if !images_url <> "" then !images_url else imcom ^ "m=IM;v="
      | 'v' -> Version.txt
      | '/' -> ""
      | c -> "%" ^ String.make 1 c

(* "copy_from_etc" is old method; rather use "Templ.copy_from_templ" *)
let rec copy_from_etc env lang imcom ic =
  let cnt = ref 0 in
  try
    while true do
      match input_char ic with
        '%' ->
          ...
          | 'r' ->
              ...
              begin match open_etc_file name with
      ...
      | '[' ->
          ...
          if c = '\n' then
            let s =
              ...
              in
              loop 0 (input_char ic)
            in
            let (s, alt) =
              Translate.inline lang '%' (macro_etc env imcom) s
            in
            let s = if alt then tnf s else s in Wserver.wprint "%s" s
          else Wserver.wprint "[%c" c
...
let get_server_string_aux cgi request =
...
let get_request_string_aux cgi request =
...
let get_server_string conf = get_server_string_aux conf.cgi conf.request
let get_request_string conf = get_request_string_aux conf.cgi conf.request
...
let url_no_index conf base pwd =
  ...
  let nenv =
    if not (ocv = "0") && nocv = "0" then ("oc", ocv) :: nenv else nenv
  in
  let nenv = List.rev nenv in
  let suff =
    List.fold_right
  ...
  in
  let cgipwd =
    if pwd && not (conf.cgi_passwd = "") then "_" ^ conf.cgi_passwd else ""
  in
  let suff =
    if conf.cgi then "b=" ^ conf.bname ^ cgipwd ^ ";" ^ suff else suff
  in
  addr ^ "?" ^ suff
  ...
  if conf.wizard || conf.just_friend_wizard then
    let print_file fname =
      ...
      match try Some (Secure.open_in fname) with Sys_error _ -> None with
        Some ic ->
          begin try
            while true do Wserver.wprint "%c" (input_char ic) done
...
let compilation_time_hook = ref (fun _ -> "")
let compilation_time conf =
  match p_getenv conf.base_env "display_compilation_time" with
    Some "on" -> !compilation_time_hook conf
  | _ -> ""
...
let target =
  ".ABlCaDmEnFbGoHIpJcKLqMNdrOP seQtRSfT_UzVuWgXvYZwh01i234j56yk78x9"

let transcode s source target =
  let rec copy_code_in s1 i =
    if i < String.length s then
      let c =
        match String.index_opt source s.[i] with
          Some j -> target.[j]
        | None -> s.[i]
      in
      Bytes.set s1 i c; copy_code_in s1 (succ i)
    else Bytes.unsafe_to_string s1
  in
  copy_code_in (Bytes.create (String.length s)) 0


let scramble pwd = transcode pwd source target

let unscramble pwd = transcode pwd target source
...
let my_string_of_float f = sprintf "%.6g" f
...
let branch_of_sosa conf base ip n =
  if Num.eq n Num.zero then invalid_arg "branch_of_sosa";
  let rec expand bl n =
...
let print_pre_center sz txt =
  for i = 1 to (sz - pre_text_size txt) / 2 do Wserver.wprint " " done;
  Wserver.wprint "%s\n" txt
...
let print_pre_right sz txt =
  ...
  if tsz < sz / 2 - 1 then
    begin
      for i = 1 to sz / 2 do Wserver.wprint " " done;
      for i = 1 to (sz / 2 - 1 - tsz) / 2 do Wserver.wprint " " done;
      ()
    end
  else for i = 1 to sz - pre_text_size txt - 1 do Wserver.wprint " " done;
  Wserver.wprint " %s\n" txt
...
let print_in_columns conf ncols len_list list wprint_elem =
  begin_centered conf;
  Wserver.wprint "<table width=\"95%%\" border=\"%d\">\n" conf.border;
  Wserver.wprint "<tr align=\"%s\" valign=\"top\">\n" conf.left;
  begin let _ =
    List.fold_left
      (fun (list, first) len ->
         let rec loop n list =
           if n = 0 then
             begin Wserver.wprint "</ul>\n</td>\n"; list, false end
           else
             match list with
               (kind, ord, elem) :: list ->
                 if n = len then
                   Wserver.wprint "<td width=\"%d\">\n" (100 / ncols)
                 else if !kind <> Elem then Wserver.wprint "</ul>\n";
                 if !kind <> Elem then
                   begin
...
let print_reference conf fn occ sn =
  Wserver.wprint "<span class=\"reference\">";
  Wserver.wprint " (%s %s.%d %s)" (transl conf "reference key")
