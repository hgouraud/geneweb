...
let highlight_diff arr diff_arr =
  let rec loop i s =
    if i >= Array.length arr then s
      ...
      let j = ref i in
      let accu = ref s in
      accu := !accu ^ "<span class=\"diff_highlight\">";
...
and eval_str_gen_record conf base env (bef, aft, p_auth) =
  ...
  | "birth_src" ->
      if p_auth then
        ...
        let a = aft.gen_p.birth_src in diff_string b a
      else "", ""
  | "baptism" ->
      if p_auth then
  ...
  | "baptism_src" ->
      if p_auth then
        ...
        let a = aft.gen_p.baptism_src in diff_string b a
      else "", ""
  | "death" ->
      if p_auth then
  ...
  | "death_src" ->
      if p_auth then
        ...
        let a = aft.gen_p.death_src in diff_string b a
      else "", ""
  | "burial" ->
      if p_auth then
  ...
  | "burial_src" ->
      if p_auth then
        ...
        let a = aft.gen_p.burial_src in diff_string b a
      else "", ""
  | "notes" ->
      if p_auth && not conf.no_note then
        let b = bef.gen_p.notes in let a = aft.gen_p.notes in diff_string b a
      else "", ""
  | "psources" ->
      if p_auth then
          ...
          if m_auth then
            match bef, aft with
              Some b, Some a ->
                ...
                let a = a.marriage_src in diff_string b a
      ...
      begin match get_env "fam" env with
        Vfam (bef, aft, m_auth) ->
          if m_auth && not conf.no_note then
            match bef, aft with
              Some b, Some a ->
                let b = b.comment in let a = a.comment in diff_string b a
