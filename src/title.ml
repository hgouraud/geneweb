...
let give_access_someone conf base (x, t) list =
  ...
  let has_dates =
    ...
    | _ -> false
  in
  if has_dates then Wserver.wprint "<em>";
  ...
  | Tname n, _, nn :: _ ->
      Wserver.wprint "%s <em>%s</em> %s" (sou base n) (sou base nn)
        (p_surname base x)
  | Tname n, _, [] -> Wserver.wprint "%s %s" (sou base n) (p_surname base x)
  ...
  if List.mem x list then Wserver.wprint "</em>" else Wserver.wprint "</a>"
...
let give_access_all_titles conf t absolute =
  Wserver.wprint "<a href=\"%sm=TT;sm=S;t=%s%s\">" (commd conf)
...
let give_access_all_places conf t =
  Wserver.wprint "<a href=\"%sm=TT;sm=S;p=%s\">" (commd conf) (code_varenv t);
  Wserver.wprint "... %s" t;
  Wserver.wprint "</a>\n"
...
let print_title_place_list conf base t p t_equiv list =
  ...
  let title h =
    if h || absolute then
      begin Wserver.wprint "%s" t; if p <> "" then Wserver.wprint " %s" p end
    else
      list_iter_first
        (fun first t ->
...
let print_places_list conf base t t_equiv list =
  ...
  let absolute = p_getenv conf.env "a" = Some "A" in
  let wprint_elem p =
    Wserver.wprint "<a href=\"%sm=TT;sm=S;t=%s;p=%s%s\">" (commd conf)
...
let print_titles conf base p =
  ...
  if List.length list > 1 then
    begin
      Wserver.wprint "<a href=\"%sm=TT;sm=A;p=%s\">" (commd conf)
        (code_varenv p);
...
let print_all_titles conf base =
  ...
  let wprint_elem (t, cnt) =
    give_access_all_titles conf t false; Wserver.wprint " (%d)" cnt
  in
  header conf title;
  wprint_in_columns conf order wprint_elem list;
  trailer conf
