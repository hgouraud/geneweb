...
let print_cousins_lev conf base max_cnt p lev1 lev2 =
  ...
  let some =
    let rec loop sosa some print_tips =
      if !cnt < max_cnt && Num.gt last_sosa sosa then
        let some =
...
let print_cousins conf base p lev1 lev2 =
  let title h =
    ...
    if lev1 = lev2 then
      ...
      let s =
        transl_a_of_gr_eq_gen_lev conf (transl conf "nephews and nieces")
          (txt_fun raw_access conf base p)
      in
      Wserver.wprint "%s" (capitale (Util.translate_eval s))
    else
  ...
  Wserver.wprint "<p>\n";
  if !cnt >= max_cnt then Wserver.wprint "etc...\n"
  else if !cnt > 1 then
    Wserver.wprint "%s: %d %s.\n" (capitale (transl conf "total")) !cnt
...
let print_anniv conf base p dead_people level =
  ...
  in
  let mode () =
    Wserver.wprint "<input type=\"hidden\" name=\"m\" value=\"C\"%s>\n"
      conf.xhs;
    Wserver.wprint "<input type=\"hidden\" name=\"i\" value=\"%d\"%s>\n"
      (Adef.int_of_iper (get_key_index p)) conf.xhs;
    Wserver.wprint "<input type=\"hidden\" name=\"t\" value=\"%s\"%s>\n"
