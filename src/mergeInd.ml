...
let print_differences conf base branches p1 p2 =
      ...
      begin
        ...
        begin
          ...
          begin
            Wserver.wprint "<li>\n";
            Wserver.wprint
              "<input type=\"radio\" name=\"%s\" value=\"1\"%s%s>\n" name chk1
              conf.xhs;
            Wserver.wprint "%s\n" x1;
            Wserver.wprint "</li>\n"
          end;
          begin
            Wserver.wprint "<li>\n";
            Wserver.wprint
              "<input type=\"radio\" name=\"%s\" value=\"2\"%s%s>\n" name chk2
              conf.xhs;
            Wserver.wprint "%s\n" x2;
            Wserver.wprint "</li>\n"
          end;
  ...
  Wserver.wprint "<input type=\"hidden\" name=\"m\" value=\"MRG_IND_OK\"%s>\n"
    conf.xhs;
  Wserver.wprint "<input type=\"hidden\" name=\"i1\" value=\"%d\"%s>\n"
...
let compatible_ind base p1 p2 =
  ...
  compatible_titles (get_titles p1) (get_titles p2) && get_rparents p2 = [] &&
  get_related p2 = [] &&
...
let effective_merge_ind conf base warning p1 p2 =
  ...
  let key =
    Name.lower (sou base p1.first_name), Name.lower (sou base p1.surname),
    p1.occ
  in
  let pgl =
    ...
    let pgl = Notes.links_to_ind conf base db key in pgl
  in
...
let propose_merge_fam conf base branches fam1 fam2 p1 p2 =
  ...
  Wserver.wprint "<ul>\n";
  html_li conf;
  Wserver.wprint "<a href=\"%s%s\">" (commd conf) (acces conf base p1);
  Merge.print_someone conf base p1;
  Wserver.wprint "</a>";
  Wserver.wprint "\n%s\n" (transl conf "with");
  Wserver.wprint "<a href=\"%s%s\">" (commd conf) (acces conf base p2);
  Merge.print_someone conf base p2;
  Wserver.wprint "</a>";
  Wserver.wprint "\n";
  Wserver.wprint "</ul>\n";
...
let print_merged conf base wl p =
  ...
  begin match p_getenv conf.env "m", p_getint conf.env "ip" with
    Some "MRG_DUP_IND_Y_N", Some ip ->
      ...
      in
      Wserver.wprint "<p>\n";
      Wserver.wprint "<a href=%sm=MRG_DUP;ip=%d%s%s>" (commd conf) ip s1 s2;
