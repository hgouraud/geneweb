...
let get_data conf =
  ...
  | Some "src" ->
      ["bi", Person get_birth_src; "bp", Person get_baptism_src;
       ...
       "p", Person get_psources; "ma", Family get_marriage_src;
...
let print_title conf base ini len =
  ...
  if ini = "" then Wserver.wprint " (%d %s)" len title
  else
    begin
      Wserver.wprint " - ";
      Wserver.wprint (fcapitale (ftransl conf "%d %s starting with %s")) len
        title ini
    end
...
let print_long conf base list len =
  ...
  List.iter
    (fun (ini_k, list) ->
       Wserver.wprint "<li>\n";
       Wserver.wprint "<a id=\"%s\">" ini_k;
       Wserver.wprint "%s" (no_html_tags ini_k);
       Wserver.wprint "</a>\n";
       Wserver.wprint "<ul class=\"mod_data_ul\">\n";
       begin let list =
              ...
              if k <> env_keys then
                let k =
                  List.fold_left
                    ...
                    "" k
                in
                Wserver.wprint "<a href=\"%sm=MOD_DATA;data=%s;%s;s=%s#mod\">"
                  (commd conf) data k (code_varenv ini);
                Wserver.wprint "%s" (quote_escaped s);
                Wserver.wprint "</a>"
              else
                begin
                  Wserver.wprint "<table class=\"mod_data_table\">\n";
                  begin
                    Wserver.wprint "<tr>\n";
                    begin
                      Wserver.wprint "<td>\n";
...
let print_short conf base list len =
  ...
  Wserver.wprint "<p class=\"list_ini\">\n";
  List.iter
    (fun s ->
       Wserver.wprint "<a href=\"%sm=MOD_DATA;data=%s;s=%s\">" (commd conf)
         data (code_varenv s);
  ...
  if nb_pers <> 0 && data_modified then
    begin
      ...
      if p_getenv conf.base_env "history" = Some "yes" then
        begin
          Wserver.wprint "<a href=\"%sm=HIST;k=20\">" (commd conf);
          Wserver.wprint "%s."
    ...
    print_link_to_welcome conf true;
    Wserver.wprint "<p>\n";
    Wserver.wprint "<a href=\"%sm=MOD_DATA;data=%s;s=%s\">" (commd conf) data
      ini;
    Wserver.wprint "%s" (capitale (transl conf "new modification"));
    Wserver.wprint "</a>";
    Wserver.wprint "</p>\n";
