    ...
    begin
      ...
      let (_, ages_sum, ages_nb) =
        List.fold_left
          (fun (last_month_txt, ages_sum, ages_nb) (p, d, cal) ->
             ...
             Wserver.wprint "<li>";
             begin let img =
               if get_access p = Friend then
                 Printf.sprintf "<img src=\"%s/%s\" %s alt=\"(f) \"%s>"
                   (Util.image_prefix conf) "friend.gif"
                   "width=\"16\" height=\"16\"" conf.xhs
               else "  "
             in
             ...
             if get_access p = Friend then Wserver.wprint " (Ami)";
             Wserver.wprint "</li>\n";
      ...
      if snd ages_nb >= 3 then
        Wserver.wprint "%s (%s) : %s<br%s>\n"
          ...
          conf.xhs;
      Wserver.wprint "<br%s>\n" conf.xhs;
      Wserver.wprint "<div align=\"center\">\n";
      Wserver.wprint "<hr width=\"50%%\"%s>\n" conf.xhs;
      Wserver.wprint "</div>\n";
      Wserver.wprint "<br%s>\n" conf.xhs;
      let by =
      ...
      Wserver.wprint "<input type=\"hidden\" name=\"m\" value=\"LD\"%s>\n"
        conf.xhs;
      begin let ds =
        Printf.sprintf
...
let print_population_pyramid conf base =
  ...
  let string_of_nb n =
    Num.to_string_sep (transl conf "(thousand separator)") (Num.of_int n)
  in
  let title _ =
  ...
  let print_image doit sex iname =
    Wserver.wprint "<td>";
    if doit then
      Wserver.wprint "<img src=\"%s/%s\" alt=\"%s\" title=\"%s\"%s>\n"
  ...
  begin let c = " cellspacing=\"0\" cellpadding=\"0\"" in
    ...
    for i = first_interv downto 0 do
      ...
      let nb_wom = wom.(i) in
      Wserver.wprint "<tr>\n";
      Wserver.wprint "<td class=\"pyramid_year\">";
      ...
      Wserver.wprint "&nbsp;";
      Wserver.wprint "</td>\n";
      Wserver.wprint "<td align=\"right\">\n";
      Wserver.wprint "<table %s>\n" c;
      Wserver.wprint "<tr>\n";
      Wserver.wprint "<td class=\"pyramid_nb\">";
    ...
    Wserver.wprint "</table id=\"table_pop_pyr\">\n"
  end;
  Wserver.wprint "</div>\n";
  let sum_men = Array.fold_left (+) 0 men in
  let sum_wom = Array.fold_left (+) 0 wom in
  Wserver.wprint "<p>\n";
  Wserver.wprint "%s %s" (capitale (transl conf "number of living persons:"))
    (string_of_nb (sum_men + sum_wom));
  Wserver.wprint "</p>\n";
  Wserver.wprint "<p>\n";
  Wserver.wprint "<form method=\"get\" action=\"%s\">\n" (commd conf);
  hidden_env conf;
  Wserver.wprint "<input type=\"hidden\" name=\"m\" value=\"POP_PYR\"%s>\n"
    conf.xhs;
  Wserver.wprint "<input type=\"hidden\" name=\"int\" value=\"%d\"%s>\n"
    interval conf.xhs;
  Wserver.wprint "<input type=\"hidden\" name=\"lim\" value=\"%d\"%s>\n" limit
    conf.xhs;
  Wserver.wprint "%s\n" (transl_nth conf "year/month/day" 0);
  Wserver.wprint "<input name=\"y\" value=\"%d\" size=\"5\"%s>\n" at_year
    conf.xhs;
  Wserver.wprint "</form>\n";
  Wserver.wprint "</p>\n";
  Hutil.trailer conf
