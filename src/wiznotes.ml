...
let print_wizards_by_alphabetic_order conf list =
  let wprint_elem (wz, (wname, (_, islash)), wfile, stm) =
    ...
    in
    if with_link then
      Wserver.wprint "<a href=\"%sm=WIZNOTES;f=%s%t\">" (commd conf)
        (Util.code_varenv wz)
        (fun _ ->
  ...
  let _ =
    List.fold_left
      (fun (spl, prev) (wz, (wname, _), wfile, stm) ->
         ...
         Wserver.wprint "%s%t" (if prev = None || new_item then "" else ",\n")
           (fun _ ->
...
let print_old_wizards conf list =
  if list = [] then ()
    ...
    begin
      Wserver.wprint "<dl>\n";
      begin
        Wserver.wprint "<dd style=\"list-style-type:circle\">\n";
          ...
          begin
            ...
            Mutil.list_iter_first
              (fun first wz ->
                 ...
                 Wserver.wprint "<a href=\"%sm=WIZNOTES;f=%s\">" (commd conf)
                   (Util.code_varenv wz);
...
let wizard_page_title conf wz wizname h = Wserver.wprint "%s" wizname
...
let print_whole_wiznote conf base auth_file wz wfile (s, date) ho =
  ...
  Wserver.wprint "<table border=\"0\" width=\"100%%\">\n";
  Wserver.wprint "<tr>\n";
  Wserver.wprint "<td>\n";
    ...
    List.fold_left
      (fun (not_everybody, first) (wz, tm_user) ->
         if wz <> conf.user && List.mem wz denying && not conf.manitou then
           ...
           begin
             begin
               Wserver.wprint "<li style=\"list-style-type:%s\">\n"
