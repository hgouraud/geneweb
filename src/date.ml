...
let string_of_on_prec_dmy_aux conf code_year sy d =
  match d.prec with
    Sure ->
...
let string_of_on_prec_dmy conf code_dmy sy d =
  let r = string_of_on_prec_dmy_aux conf code_dmy sy d in
  replace_spaces_by_nbsp r

let string_of_on_dmy conf d =
  let sy = code_dmy conf d in string_of_on_prec_dmy conf code_year sy d
...
let string_of_prec_dmy conf s d =
  match d.prec with
    Sure -> nominative s
...
let print_dates conf base p =
  ...
  let burial_date_place cod =
    ...
    | None -> if place <> "" then Wserver.wprint " -&nbsp;"
    end;
    if place <> "" then Wserver.wprint "%s" (string_of_place conf place)
  in
