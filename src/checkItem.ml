...
type base_warning = (person, family, title) warning
...
*)
(* Constants used for computing the warnings. *)
let max_age_btw_cpl = 75     (* De nombreux centenaires épousent des jeunes filles *)
let max_days_btw_sibl = 6    (* plus restrictif : 6 jours entre des jumeaux *)
let max_month_btw_sibl = 4   (* plus tolérant : naissance de prématuré 4 mois après *)
...
let max_father_age = 83      (* âge dépassé par de nombreux pères sur Roglo *)
let max_mother_age = 58
let lim_date_marriage = 1749 (* L'âge au mariage n'est contrôlé que pour les mariages postérieurs à 1749 *)
...
let birth_before_baptism base warning p =
  match
    Adef.od_of_codate (get_birth p), Adef.od_of_codate (get_baptism p)
  with
    Some d1, Some d2 ->
      if strictly_after d1 d2 then warning (BirthAfterBaptism p)
  | _ -> ()
...
let check_person_dates_as_witness base warning p =
          ...
          if get_sex c = Male then
            Array.iter
              (fun ifam ->
                 ...
                 if Mutil.array_mem (get_key_index p) (get_witnesses fam) then
                   list := (ifam, fam) :: !list)
  ...
  List.iter
    (fun (ifam, fam) ->
       check_normal_marriage_date_for_someone base (fun () -> ()) warning true
         fam ip)
    list
