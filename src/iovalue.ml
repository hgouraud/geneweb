...
let rec input_loop ifuns ic =
  ...
  if code >= 0x40 then
    ...
    match code with
      0x0 -> Obj.magic (sign_extend (ifuns.input_byte ic))
    | 0x1 ->
...
let gen_output_block_header ofuns oc tag size =
  ...
  if tag < 16 && size < 8 then ofuns.output_byte oc (0x80 + tag + size lsl 4)
  else if Sys.word_size = 64 && hd >= 1 lsl 32 then
    begin
      ofuns.output_byte oc 0x13;
      for i = 1 to 8 do
...
let rec output_loop ofuns oc x =
  if Obj.is_int x then
    if Obj.magic x >= 0 && Obj.magic x < 0x40 then
      ofuns.output_byte oc (0x40 + Obj.magic x)
    else if Obj.magic x >= -128 && Obj.magic x < 128 then
      begin ofuns.output_byte oc 0x0; ofuns.output_byte oc (Obj.magic x) end
    else if Obj.magic x >= -32768 && Obj.magic x < 32768 then
      begin
        ofuns.output_byte oc 0x1;
        ofuns.output_byte oc (Obj.magic x lsr 8);
        ofuns.output_byte oc (Obj.magic x)
      end
    else
      begin
        ofuns.output_byte oc 0x2;
        ofuns.output_binary_int oc (Obj.magic x)
      end
  else if Obj.tag x = Obj.string_tag then
    let len = String.length (Obj.magic x) in
    if len < 0x20 then ofuns.output_byte oc (0x20 + len)
    else if len < 0x100 then
      begin ofuns.output_byte oc 0x9; ofuns.output_byte oc len end
    else begin ofuns.output_byte oc 0xA; ofuns.output_binary_int oc len end;
