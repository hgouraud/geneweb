...
(* remove Filename.basename from those two functions *)
...
let old_source_file_name conf fname =
  let bname = conf.bname in
  let lang = conf.lang in
  let fname1 =
    List.fold_right Filename.concat [Util.base_path ["src"] bname; lang]
...
and copy_from_file conf base name mode =
  let fname =
    ...
    | Source -> new_source_file_name conf name
  in
...
let new_print_source = gen_print true Source

let () =
  Gwd_lib.GwdPlugin.register ~ns:"doc" [ "DOC", fun assets conf base ->
      ...
      | Some f ->
          begin
            if Filename.check_suffix f ".txt" then
              ...
              new_print_source conf base f
            else new_print_source_image conf f;
            true
          end
      | None -> false
      end
