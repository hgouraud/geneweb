...
const RelationMatrix = (() => {
  ...
  function generateSumLinks(sortedPaths, p1Iper, p2Iper) {
    ...
    html += '<div class="text-center mx-3">';
    
    const entries = multiLevel.map(anc => {
      const l1s = [...new Set(anc.levels.map(l => l.l1))].sort((a, b) => b - a).join(',');
      const l2s = [...new Set(anc.levels.map(l => l.l2))].sort((a, b) => b - a).join(',');
      const url = '?m=RL&i=' + anc.iper + '&i1=' + p1Iper + '&i2=' + p2Iper + 
                  '&l1=' + l1s + '&l2=' + l2s + '&dag=on';
      
      let result = '<span class="text-nowrap"><a href="?i=' + anc.iper + '">' + 
                   anc.name + '</a> [<a href="' + url + '">' + anc.total + '</a>]</span>';
      
      if (anc.partner) {
        const pUrl = '?m=RL&i=' + anc.partner.iper + '&i1=' + p1Iper + '&i2=' + p2Iper + 
                     '&l1=' + l1s + '&l2=' + l2s + '&dag=on';
        result += ' <span class="text-nowrap">&amp; <a href="?i=' + anc.partner.iper + '">' + 
                  anc.partner.name + '</a> [<a href="' + pUrl + '">' + anc.partner.total + '</a>]</span>';
      }
