...
let input bname =
  ...
  let base_data =
    {persons = persons; ascends = ascends; unions = unions;
     visible =
       {v_write =
         (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 218, 27)));
        v_get =
          (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 218, 43)))};
     families = families; couples = couples; descends = descends;
     strings = strings; particles = []; bnotes = bnotes; bdir = bname}
  in
  let base_func =
    {person_of_key =
      (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 223, 21)));
     persons_of_name =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 223, 47)));
     strings_of_fsname =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 224, 25)));
     persons_of_surname =
       {find =
         (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 225, 34)));
        cursor =
          (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 225, 51)));
        next =
          (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 225, 66)))};
     persons_of_first_name =
       {find =
         (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 226, 37)));
        cursor =
          (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 226, 54)));
        next =
          (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 226, 69)))};
     patch_person =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 227, 20)));
     patch_ascend =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 227, 43)));
     patch_union =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 228, 19)));
     patch_family =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 228, 42)));
     patch_couple =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 228, 65)));
     patch_descend =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 229, 21)));
     patch_name =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 229, 42)));
     insert_string =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 229, 66)));
     commit_patches =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 230, 22)));
     commit_notes = commit_notes;
     patched_ascends =
       (fun _ -> raise (Match_failure ("./gwtp/iolight.ml4", 231, 23)));
