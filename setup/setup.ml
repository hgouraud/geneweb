...
let charset conf =
  try Hashtbl.find conf.lexicon " !charset" with Not_found -> "iso-8859-1"

let nl () = Wserver.wprint "\013\010"
...
let trailer conf =
  ...
  Wserver.wprint "<em>\n";
  Wserver.wprint
    "<a href=\"https://github.com/geneweb/geneweb/\"><img src=\"images/logo_bas.png\" align=\"absmiddle\" style = \"border: 0\" /></a> Version %s Copyright &copy 1998-2016\n</em>\n"
    Version.txt;
  Wserver.wprint "</div>\n";
  Wserver.wprint "</div>\n";
...
let parse_upto lim =
  let rec loop len (strm__ : _ Stream.t) =
    ...
    | Some c ->
        Stream.junk strm__;
        begin try loop (Buff.store len c) strm__ with
...
let get_variable strm =
  let rec loop len =
    ...
    | Some c -> Stream.junk strm__; loop (Buff.store len c)
    | _ -> raise Stream.Failure
  in
  loop 0
...
let get_binding strm =
  let rec loop len =
    ...
    match Stream.peek strm__ with
      Some '=' ->
        Stream.junk strm__; let k = Buff.get len in k, get_variable strm
    | Some c -> Stream.junk strm__; loop (Buff.store len c)
    | _ -> raise Stream.Failure
  in
  loop 0
    ...
    let rec loop (vlist, flist) =
      ...
      match Stream.peek strm__ with
        Some '%' ->
          ...
          in
          loop (vlist, flist)
      | Some _ -> Stream.junk strm__; loop (vlist, flist)
      | _ -> vlist, flist
    in
    loop ([], [])
  in
...
let translate_phrase lang lexicon s n =
  let n =
    match n with
      Some n -> n
    | None -> 0
  ...
  try
    while true do
        ...
        '[' ->
          ...
          | _ ->
              ...
              let n =
                ...
                match Stream.peek strm__ with
                  Some ('0'..'9' as c) ->
                    Stream.junk strm__; Some (Char.code c - Char.code '0')
                | _ -> None
              in
      ...
      | '%' ->
          ...
          | 't' -> print_if conf print false strm
          | 'v' ->
    ...
    function
      (d, x) :: list ->
        ...
        List.iter
          (fun (k, v) ->
             if k = "sel" then ()
             else begin print k; print "="; print v; print ";" end)
          conf.env;
        print "sel=";
        print (code_varenv d);
        print "\">";
...
let simple conf =
  ...
  in
  let ged =
    if Filename.check_suffix (String.lowercase ged) ".ged" then ged else ""
  in
  let out_file =
...
let delete_1 conf =
  List.iter (fun (k, v) -> if v = "del" then rm_base (k ^ ".gwb")) conf.env;
...
let gwf_1 conf =
  ...
  List.iter
    ...
    vars;
  List.iter
    (fun (k, v) -> if List.mem k vars then () else fprintf oc "%s=%s\n" k v)
    benv;
  close_out oc;
...
let gwd_1 conf =
  ...
  let print_param k =
    ...
    | _ -> ()
  in
  begin match p_getenv conf.env "setup_link" with
...
let wrap_setup a b c = (); try setup a b c with Exit -> ()
  ...
  match try Some (open_out fname) with Sys_error _ -> None with
    Some oc ->
      ...
      List.iter
        ...
        env;
      if not !lang_found then fprintf oc "-lang\n%s\n" lang;
      close_out oc
  | None -> ()
...
let main () = intro (); Wserver.f None !port 0 None wrap_setup
