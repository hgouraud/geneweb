...
let warning_use_has_parents_before_parent (_bp, _ep) var r =
  if Sys.unix then
    begin
      Printf.eprintf "\
  ...
  let rec loop1 first =
    ...
    | pl :: t_pl_l ->
        ...
        if link_to_ind && cnt < max_rlm_nbr then
          begin
          Output.printf conf " (<a href=\"%sm=PS&t=L&tt=%s&%s%s&nb=%d%s" (commd conf)
            p1
...
let print_html_places_surnames conf base max_rlm_nbr link_to_ind
  ...
  let print_sn (sn, ips) so =
    ...
    if link_to_ind && List.length ips < max_rlm_nbr then
      begin
        Output.printf conf " (<a href=\"%sm=PS&t=L&tt=%s&%s%s&nb=%d" (commd conf)
          so
          ("&k=" ^ (Mutil.encode so))
          opt len ;
  ...
  let rec loop prev =
    ...
    | ((so, pl), snl) :: list ->
        ...
        in
        loop1 prev pl;
        if List.length pl = 1 then Output.print_string conf "<ul>\n";
...
let print_all_places_surnames_short conf base ~add_birth ~add_baptism ~add_death ~add_burial ~add_marriage =
  ...
  print_aux conf title begin fun () ->
    ...
    Array.iteri
      (fun i (s, x) ->
         Output.printf conf "<a href=\"%sm=PS&t=PS_HG%s&k=%s\">%s</a> (%d)%s"
...
(* ************************************************ *)
let () =
  Gwd_lib.GwdPlugin.register ~ns:"places" [ "PS", fun assets conf base ->
    ...
    | Some "L" ->
          ...
          in
          Hutil.interp_no_header conf "list"
