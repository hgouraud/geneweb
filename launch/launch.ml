...
type state =
  { tk_win : Widget.widget;
    mutable config_env : (string * string) list;
    mutable bin_dir : string;
    mutable sys_dir : string;
    mutable doc_dir : string;
    mutable port : int;
    mutable browser : string option;
    mutable bases_dir : string;
    mutable browser_lang : bool;
    mutable digest_auth : bool;
    mutable server_running : int option;
    mutable waiting_pids : int list }

let trace = ref false
let config_file = Filename.concat "gw" "config.txt"
let lexicon_file = Filename.concat "gw" "launch_lex.txt"
...
let close_server state =
  match state.server_running with
    Some server_pid ->
      ...
      let _ = (Unix.waitpid [] server_pid : int * _) in
      state.server_running <- None; eprintf "\n"; flush stderr
  | None -> ()

let clean_waiting_pids state =
  state.waiting_pids <-
    List.filter
      (fun pid ->
...
let browse state browser port dbn =
  let pid =
    match browser with
      Some browser ->
        exec browser [sprintf "http://localhost:%d/%s" port dbn] Unix.stdout
          Unix.stderr
    | None -> -1
  in
  if pid = -1 then
...
let button_create wid params comm = Button.create wid (Command comm :: params)

let config state title v def select to_string from_string set prev next =
  match try Some (from_string (v ())) with Failure _ | Not_found -> None with
    Some v -> set state v; next state
  | None ->
      ...
      let kont f state =
        match
          try Some (from_string (Textvariable.get var)) with Failure _ -> None
        with
          Some d -> Pack.forget [gframe]; set state d; f state
        | None -> ()
      in
      let buts =
        match prev with
          Some (prev_var, prev) ->
            ...
            in
            pack [but1] [Side Side_Left]; pack [but2] [Side Side_Right]; buts
        | None ->
...
let rec show_main state =
  ...
  if databases = [] then
    ...
    begin let txt =
      Label.create run_frame [Text (transl "Available databases:")]
    in
      pack [txt] [];
      List.iter
        (fun dbn ->
  ...
  let cbut =
    Button.create run_frame
      [Text (transl "Create a new database");
       Command (fun _ -> Pack.forget [gframe]; new_database state)]
  in
  let obut =
    Button.create run_frame
      [Text (transl "Change options");
       Command (fun _ -> Pack.forget [gframe]; change_options state)]
  in
  let rbut =
    Button.create run_frame
      [Text (transl "Restart");
  ...
  let buts =
    ...
    let but1 =
      button_create buts [Text (transl "Cancel")]
        (fun _ -> Pack.forget [gframe]; show_main state)
    in
    let but2 =
             ...
             let rec loop i =
               if i = String.length s then
                 ...
                 if Sys.file_exists (db ^ ".gwb") then ()
                   ...
                   Pack.forget [gframe]; show_main state
               else
                 match s.[i] with
...
let rec config_bases_dir state =
  config state (fun () -> transl "Databases directory:")
    ...
    (fun state ->
       ...
       let config_env_bases_dir =
         try List.assoc "bases_dir" state.config_env with Not_found -> ""
       in
...
and config_browser state =
  ...
  let select frame var =
    ...
    let list =
      if browsers = [] then
        ...
        Textvariable.set var "other"; bro
      else
        ...
        List.iter
          (fun fn ->
             ...
             in
             pack [rad] [Side Side_Left]; pack [frad] [Fill Fill_X])
          browsers;
        let frad = Frame.create list [] in
  ...
  let from_string s =
    ...
    in
    let s = if s = "other" then "" else s in if s = "" then None else Some s
  in
  config state
    (fun () ->
       if Lazy.force browsers = [] then transl "Browser:"
       else transl "Browser(s):")
    (fun () -> List.assoc "browser" state.config_env) None select to_string
    from_string (fun state browser -> state.browser <- browser)
    (Some ("port", config_port))
    (fun state ->
       let browser = state.browser in
       begin match browser with
       ...
       begin match browser with
         Some browser ->
           let config_env_browser =
             try List.assoc "browser" state.config_env with Not_found -> ""
           in
           if browser <> "" && browser <> config_env_browser then
             begin
               state.config_env <-
...
and config_port state =
  config state (fun () -> transl "Port:")
    ...
    (fun frame var ->
       ...
       Entry.selection_to ent End; ent)
    string_of_int
    (fun s ->
       let i = int_of_string s in
       if i < 1024 then failwith "bad value" else i)
    (fun state port -> state.port <- port) (Some ("sys_dir", config_sys_dir))
    (fun state ->
       ...
       in
       if port <> config_env_port || not (Sys.file_exists config_file) then
         begin
           state.config_env <-
...
and config_sys_dir state =
  config state (fun () -> transl "GeneWeb system directory:")
    ...
    (fun frame var ->
       ...
       in
       pack [lab; but] []; sframe)
    (fun s -> s) (fun s -> s) (fun state sys_dir -> state.sys_dir <- sys_dir)
    ...
    (fun state ->
       ...
       let config_env_sys_dir =
         try List.assoc "sys_dir" state.config_env with Not_found -> ""
       in
...
and config_bin_dir state =
  config state (fun () -> transl "GeneWeb binary directory:")
    ...
    (fun frame var ->
       ...
       in
       pack [lab; but] []; sframe)
    (fun s -> s) (fun s -> s) (fun state bin_dir -> state.bin_dir <- bin_dir)
    None
    (fun state ->
       ...
       let config_env_bin_dir =
         try List.assoc "bin_dir" state.config_env with Not_found -> ""
       in
...
let default_bin_dir = "../src"
let default_sys_dir = "../hd"
let default_doc_dir = "../doc"
let default_port = 2317
let default_browser = None
let default_bases_dir = "../../gwbases"

let speclist = ["-trace", Arg.Set trace, " Trace server"]
let anon_fun s = raise (Arg.Bad (sprintf "Don't know what to do with %s" s))
let usage_msg = "Usage: launch [option]"
