...
let check_month m =
  if m < 1 || m > 12 then
    begin
      fprintf !log_oc "Bad (numbered) month in date: %d\n" m;
      flush !log_oc
    end
  ...
  let f =
    family_of_gen_family
      {marriage = Adef.codate_None; marriage_place = empty;
       marriage_src = empty; witnesses = [| |]; relation = !relation_status;
...
let capitalize_word s =
  let rec copy special i len uncap =
      ...
      | c ->
          if Char.code c < 128 then
            ...
            if nbc = 1 || nbc < 0 || i + nbc > String.length s then
              ...
              in
              let (t, j) = s, i + nbc in copy false j (Buff.mstore len t) true
  in
  copy false 0 0 (particle s 0)
         ...
         (fun r lines ->
            ...
            if r.rlab = "NOTE" && r.rval <> "" && r.rval.[0] = '@' then
              ...
              | None ->
                  ...
                  fprintf !log_oc "Note %s not found\n" addr;
                  flush !log_oc;
                  lines
  ...
  match find_field "SOUR" r.rsons with
    Some r ->
      if String.length r.rval > 0 && r.rval.[0] = '@' then
        ...
        | None ->
            print_location r.rpos;
            fprintf !log_oc "Source %s not found\n" r.rval;
            flush !log_oc;
            "", []
...
let applycase_surname s =
  ...
  | UpperCase ->
      if !charset = Utf8 then uppercase_name s else String.uppercase s
      ...
      Some r ->
        if r.rsons = [] then
          ...
          let d =
            match find_field "DATE" r.rsons with
              Some r ->
                begin match date_of_field r.rpos r.rval with
    ...
    match find_field "DIV" r.rsons with
      Some r ->
        begin match find_field "DATE" r.rsons with
          Some d -> Divorced (Adef.codate_of_od (date_of_field d.rpos d.rval))
        | _ ->
            match find_field "PLAC" r.rsons with
...
let make_gen3 gen r =
  ...
  | "NOTE" -> ()
  | "SOUR" -> ()
  | "TRLR" -> eprintf "*** Trailer ok\n"; flush stderr
  | s -> fprintf !log_oc "Not implemented typ = %s\n" s; flush !log_oc
  ...
  let rec loop () =
    ...
    | None ->
        ...
        match Stream.peek strm__ with
          Some ('1'..'9') ->
            Stream.junk strm__;
            let _ = (get_to_eoln 0 strm : string) in loop ()
        | Some _ ->
            Stream.junk strm__;
            let _ = (get_to_eoln 0 strm : string) in loop ()
        | _ -> ()
...
let check_undefined gen =
  for i = 0 to gen.g_per.tlen - 1 do
    ...
    | Left3 lab ->
        ...
        fprintf !log_oc "Warning: undefined person %s\n" lab;
  ...
  for i = 0 to gen.g_fam.tlen - 1 do
    ...
    | Left3 lab ->
        ...
        fprintf !log_oc "Warning: undefined family %s\n" lab;
      ...
      Right3 (p, a, u) ->
        ...
        then
          ...
          if fn = "?" && sn = "?" then ()
            ...
            begin
              fprintf !log_oc "Adding parents to isolated person: %s.%d %s\n"
                fn (get_occ p) sn;
...
let make_base (persons, families, strings, bnotes) bdir =
  ...
  let base_data =
    {persons = record_access_of persons; ascends = record_access_of ascends;
     ...
     visible =
       {v_write =
         (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2664, 27)));
        v_get =
          (fun _ ->
            raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2664, 43)))};
  ...
  let base_func =
    {person_of_key =
      (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2670, 21)));
     persons_of_name =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2670, 47)));
     strings_of_fsname =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2671, 25)));
     persons_of_surname =
       {find =
         (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2672, 34)));
        cursor =
          (fun _ ->
            raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2672, 51)));
        next =
          (fun _ ->
            raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2672, 66)))};
     persons_of_first_name =
       {find =
         (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2673, 37)));
        cursor =
          (fun _ ->
            raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2673, 54)));
        next =
          (fun _ ->
            raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2673, 69)))};
     patch_person =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2674, 20)));
     patch_ascend =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2674, 43)));
     patch_union =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2675, 19)));
     patch_family =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2675, 42)));
     patch_couple =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2675, 65)));
     patch_descend =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2676, 21)));
     patch_name =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2676, 42)));
     insert_string =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2676, 66)));
     commit_patches =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2677, 22)));
     commit_notes =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2677, 45)));
     patched_ascends =
       (fun _ -> raise (Match_failure ("./ged2gwb/ged2gwb.ml4", 2677, 71)));
      ...
      Some ifam ->
        ...
        if get_fam_index fam = Adef.ifam_of_int (-1) then
          ...
          if array_memq (Adef.iper_of_int i) (get_children des) then ()
            ...
            fprintf !log_oc "%s is not the child of his/her parents\n"
              (designation base p);
            fprintf !log_oc "- %s\n"
              (designation base (poi base (get_father cpl)));
            fprintf !log_oc "- %s\n"
              (designation base (poi base (get_mother cpl)));
            fprintf !log_oc "=> no more parents for him/her\n";
            fprintf !log_oc "\n";
            flush !log_oc;
            ascends.(i) <- ascend_with_parents a None
    | None -> ()
      ...
      then
        begin
          ...
          if ffn = "?" && fsn = "?" && mfn <> "?" && msn <> "?" then
            ...
            begin
              fprintf !log_oc "=> deleted this family for him/her\n";
      ...
      match get_parents a with
        Some ifam ->
          if Adef.int_of_ifam ifam <> i then
            begin
              fprintf !log_oc "Other parents for %s\n" (designation base p);
              fprintf !log_oc "- %s\n"
                (designation base (poi base (get_father cpl)));
              fprintf !log_oc "- %s\n"
                (designation base (poi base (get_mother cpl)));
              fprintf !log_oc "=> deleted in this family\n";
              fprintf !log_oc "\n";
              flush !log_oc;
              to_delete := get_key_index p :: !to_delete
            end
      | None ->
          fprintf !log_oc "%s has no parents but is the child of\n"
            (designation base p);
          fprintf !log_oc "- %s\n"
            (designation base (poi base (get_father cpl)));
          fprintf !log_oc "- %s\n"
            (designation base (poi base (get_mother cpl)));
          fprintf !log_oc "=> added parents\n";
          fprintf !log_oc "\n";
          flush !log_oc;
    ...
    then
      ...
      begin
        if get_sex fath = Female then
          fprintf !log_oc "Warning - husband with female sex: %s\n"
            (designation base fath);
        if get_sex moth = Male then
          fprintf !log_oc "Warning - wife with male sex: %s\n"
...
let neg_year_dmy =
  ...
  | {day = d; month = m; year = y; prec = YearInt y2} ->
      {day = d; month = m; year = -abs y; prec = YearInt (-abs y2); delta = 0}
        ...
        let fam =
          family_of_gen_family
            {(gen_family_of_family fam) with marriage =
              Adef.codate_of_od (Some (neg_year d))}
        in
        families.(j) <- fam
    | None -> ()
  done;
...
let finish_base base (persons, families, _, _) =
  ...
  if !do_check then
    Check.check_base base
      (fun x -> Check.print_base_error !log_oc base x; fprintf !log_oc "\n")
      (function
         UndefinedSex _ -> ()
       | x -> Check.print_base_warning !log_oc base x; fprintf !log_oc "\n")
...
let set_undefined_death_interval s =
  try
    let (strm__ : _ Stream.t) = Stream.of_string s in
    let a = number 0 strm__ in
    match Stream.peek strm__ with
      Some '-' ->
        Stream.junk strm__;
        let b =
          try number 0 strm__ with Stream.Failure -> raise (Stream.Error "")
        in
        eprintf "ay %s dy %s\n" a b;
        flush stderr;
  ...
  try main () with
    e ->
      let e =
        ...
        | _ -> e
      in
      fprintf !log_oc "Uncaught exception: %s\n" (Printexc.to_string e);
      if !log_oc != stdout then close_out !log_oc;
      exit 2
